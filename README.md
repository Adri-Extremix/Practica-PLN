...
## Baseline supervisado para Task C

Entrenamiento:

```bash
python3 train_task_c_spans.py --train-path HopeEXP_Train.jsonl --output-dir outputs/task_c_spans
```

Predicción:

```bash
python3 predict_task_c_spans.py --input-path HopeEXP_Test_unlabeled.jsonl --model-dir outputs/task_c_spans/best_model --output-path outputs/submission_task_c.json
```

## Barrido de modelos para Task B y Task C

Task B (emociones):

```bash
python3 run_model_sweep.py --task b --base-output-dir outputs/model_sweeps
```

Task C (spans):

```bash
python3 run_model_sweep.py --task c --base-output-dir outputs/model_sweeps
```

Puedes pasar argumentos extra al script de entrenamiento de cada tarea usando `--extra-args`. Por ejemplo:

```bash
python3 run_model_sweep.py --task c --extra-args --epochs 3 --batch-size 4
```

Scripts nuevos:

- `train_task_b_emotion.py`: entrena Task B desde CLI y guarda métricas/configuración.
- `predict_task_b_emotion.py`: genera predicciones de Task B desde un checkpoint o ensemble simple.
- `run_model_sweep.py`: lanza barridos de backbone para Task B o Task C.
